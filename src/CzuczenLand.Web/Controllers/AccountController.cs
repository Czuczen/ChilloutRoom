...
using CzuczenLand.Web.Controllers.Results;
...
public class AccountController : CzuczenLandControllerBase
{
    ...
    private async Task SignInAsync(User user, ClaimsIdentity identity = null, bool rememberMe = false)
    {
        ...
        if (rememberMe) {
            ...
            _authenticationManager.SignIn(
                ...
                {
                    IsPersistent = true,
                    ExpiresUtc = DateTimeOffset.UtcNow.AddMinutes(int.Parse(System.Configuration.ConfigurationManager.AppSettings["AuthSession.ExpireTimeInMinutes.WhenNotPersistent"] ?? "1440"))
                },
                identity);
        }
    }
    ...
    private Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
    {
        ...
        {
            case AbpLoginResultType.Success:
                return new ApplicationException("Nie wywołuj tej metody z wynikiem sukcesu!");
            case AbpLoginResultType.InvalidUserNameOrEmailAddress:
            case AbpLoginResultType.InvalidPassword:
                return new UserFriendlyException("Logowanie nie powiodło się", "Nieprawidłowa nazwa użytkownika lub hasło");
            case AbpLoginResultType.InvalidTenancyName:
                return new UserFriendlyException("Logowanie nie powiodło się", "Nie ma najemcy z nazwą: " + tenancyName);
            case AbpLoginResultType.TenantIsNotActive:
                return new UserFriendlyException("Logowanie nie powiodło się", "Najemca nie jest aktywny: " + tenancyName);
            case AbpLoginResultType.UserIsNotActive:
                return new UserFriendlyException("Logowanie nie powiodło się", "Użytkownik nie jest aktywny i nie może się zalogować: " + usernameOrEmailAddress);
            case AbpLoginResultType.UserEmailIsNotConfirmed:
                return new UserFriendlyException("Logowanie nie powiodło się", "Adres e-mail użytkownika nie jest potwierdzony i nie można się zalogować");
            case AbpLoginResultType.LockedOut:
                return new UserFriendlyException("Logowanie nie powiodło się", "Wiadomość zablokowana przez użytkownika");
            default: //Can not fall to default actually. But other result types can be added in the future and we may forget to handle it
                Logger.Warn("Unhandled login fail reason: " + result);
                return new UserFriendlyException("Logowanie nie powiodło się");
        }
    }
    ...
    public virtual async Task<ActionResult> Register(RegisterViewModel model)
    {
        try
        {
            ...
            if (model.IsExternalLogin)
            {
                ...
                {
                    throw new ApplicationException("Nie można zalogować się z zewnątrz!");
                }
            ...
            else
            {
                ...
                {
                    throw new UserFriendlyException("Formularz jest nieprawidłowy");
                }
            }
        ...
        {
            ViewBag.IsMultiTenancyEnabled = _multiTenancyConfig.IsEnabled;
            ViewBag.ErrorMessage = ViewHelper.GetCorrectMessage(ex, model);
    ...
    private async Task<Tenant> GetActiveTenantAsync(string tenancyName)
    {
        ...
        if (tenant == null)
        {
            throw new UserFriendlyException("Nie ma najemcy z określoną nazwą", tenancyName);
        }

        if (!tenant.IsActive)
        {
            throw new UserFriendlyException("Najemca nie jest aktywny", tenancyName);
        }
